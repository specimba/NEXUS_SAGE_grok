...
await sleep(700);
if (scrollSel === "bottom") await ev(`window.scrollTo(0, document.documentElement.scrollHeight)`);
else if (scrollSel) await ev(`document.querySelector(${JSON.stringify(scrollSel)})?.scrollIntoView({ block: 'start' })`);
await sleep(300);
