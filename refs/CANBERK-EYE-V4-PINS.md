...
Brief lane after the craft chain:

| Step | What landed | Proof |
|------|-------------|-------|
| Green-B | KPI bento + `Δ LIVE` + 8/4 Take/pins | stamped proto |
| V3 | Grain/glass/bloom still-readable | PASS vs B |
...
| Call | Means | What happens next |
|------|-------|-------------------|
| **Ship** | V4+pins is good enough as Brief baseline | Director may uncork **Voice/Digest L·mid·R parity** — still no `WIRE-*` until freeze lifts separately |
| **Rusty** | Still old / not end design | See `refs/CANBERK-RUSTY-PATH.md` (A parity · B Brief depth · C ops) or name what’s wrong |
| **Name-next** | Brief OK, different priority | You name the next beat; Voice/Digest parity stays parked unless you pick it |

## Uncork implication (if Ship)

- Uncorks craft land only for Digest `[03]` + Voice `[05]` instrument parity.
- Does **not** auto-lift HTTP freeze or approve new `WIRE-*`.

## Locks that stay either way

Skin V2 phosphor · Take = story · KPIs = meters · Sol≠Astra · free only · no `004`.

## Ask

Hard-refresh, then reply: **ship** / **rusty** / **name-next** (and if name-next, what).

### Reviewer soft-stamp — 2026-09-05T19:08Z (Reviewer Gürok)

**EYE PNG soft PASS** — `refs/VISUAL-PROOF-v4-pins-live.png` @18:51 vs live.
